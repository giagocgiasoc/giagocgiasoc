...
router.get('/cart/add-to-cart-laptop/:id', function(req, res, next) {
  if(req.useragent.isMobile){
    ...
    Laptop.findById(productId, function(err, product) {
       if (err) {
        ...
        res.redirect('/cart/shopping-cart-laptop')
    });
  }
  else{
    var productId = req.params.id;
...
router.get('/cart/shopping-cart-laptop/',function(req,res,next){
  if (!req.session.cart) {
      return res.render('fontend/laptop/shopping-cart', {products: null});
  }
...
//thanh toan offline
router.get('/cart/checkout-laptop', function(req, res, next) {
  if(req.useragent.isMobile){
    if (!req.session.cart) {
        return res.redirect('/cart/shopping-cart-laptop');
    }
    var cart = new Cart(req.session.cart);
...
router.post('/cart/checkout-laptop', function(req, res, next) {
  if(req.useragent.isMobile){
    if (!req.session.cart) {
        return res.redirect('/cart/shopping-cart-laptop');
    }
  ...
  transporter.use('compile', hbs(handlebarOptions));
        ...
        res.render('fontend/laptop/checkoutInfo',{successMsg: successMsg, noMessages: !successMsg,order:order,cart:cart.generateArray(),layout:'layouts/layoutdesktop/layoutmaytinhdetail'});
    });
  }
}
